(() => {
  ...
  var KNOWN_ENTITIES = /* @__PURE__ */ new Set([
    "valorant",
    "minecraft",
    "fortnite",
    "roblox",
  ...
  var globalSeenIds = /* @__PURE__ */ new Set();
  var lastSearchQuery = "";
  var activeSearches = {};
  ...
  chrome.runtime.onInstalled.addListener(() => {
    ...
    chrome.storage.local.get(["activeLanguages", "bubblesBurst", "isEnabled", "maxResultsPerLang"], (res) => {
      ...
      if (res.bubblesBurst === void 0) updates.bubblesBurst = 0;
      if (res.isEnabled === void 0) updates.isEnabled = true;
  ...
  function detectScript(text) {
    ...
    let maxCount = 0;
    for (const [script2, count] of Object.entries(counts)) {
      if (count > maxCount) {
        maxCount = count;
        maxScript = script2;
      }
  ...
  function classifyVideoLanguage(title, description, targetLangCode) {
    ...
    if (targetScript !== "latin") {
      const script3 = detectScript(title);
      if (script3 === targetScript) {
    ...
    const script2 = detectScript(combinedText);
    if (targetScript === "latin" && script2 === "latin") {
  ...
  function isLikelyEnglish(title, description) {
    ...
    const script2 = detectScript(title);
    if (script2 !== "latin" && script2 !== "unknown") return false;
    if (text.length >= 10) {
  ...
  function extractYtInitialData(html2) {
    const marker = "var ytInitialData = ";
    const startIdx = html2.indexOf(marker);
    if (startIdx === -1) {
      const altMarker = 'window["ytInitialData"] = ';
      const altStartIdx = html2.indexOf(altMarker);
      if (altStartIdx === -1) return null;
      const remaining2 = html2.substring(altStartIdx + altMarker.length);
      const endIdx2 = remaining2.indexOf(";<\/script>");
      if (endIdx2 === -1) return null;
      return JSON.parse(remaining2.substring(0, endIdx2));
    }
    const remaining = html2.substring(startIdx + marker.length);
  ...
  function parseVideoRenderer(vr) {
    ...
    const authorUrl = vr.ownerText?.runs?.[0]?.navigationEndpoint?.browseEndpoint?.canonicalBaseUrl || vr.ownerText?.runs?.[0]?.navigationEndpoint?.commandMetadata?.webCommandMetadata?.url || "";
  ...
  async function searchYouTube(query, langCode, region, page = 1) {
    ...
    try {
      if (page === 1) {
        ...
        const { html: html2 } = await fetchSearchPage(query, langCode, gl);
        const data2 = extractYtInitialData(html2);
        if (!data2) throw new Error("Could not parse ytInitialData from YouTube HTML");
        const keyMatch = html2.match(/"INNERTUBE_API_KEY"\s*:\s*"([^"]+)"/);
        const apiKey = keyMatch ? keyMatch[1] : null;
        const contents = data2.contents?.twoColumnSearchResultsRenderer?.primaryContents?.sectionListRenderer?.contents;
        if (contents) {
        ...
        if (cached && cached.page === page - 1 && cached.token && cached.apiKey) {
          ...
          const items = responseJson.onResponseReceivedCommands?.[0]?.appendContinuationItemsAction?.continuationItems || responseJson.continuationContents?.searchContinuation?.contents;
          if (items) {
  ...
  async function translateText(text, from, to) {
    ...
    try {
      ...
      if (data2 && data2[0]) {
        return data2[0].filter((seg) => seg && seg[0]).map((seg) => seg[0]).join("");
      }
      return text;
    } catch (err) {
  ...
  async function processGlobalSearch(searchQuery, page = 1) {
    if (searchQuery !== lastSearchQuery) {
      globalSeenIds = /* @__PURE__ */ new Set();
    ...
    if (queryInfo.translate) {
      ...
      translations.forEach((t) => {
        translatedQueries[t.code] = t.translated;
      });
  ...
  function parsePublishedTextToSeconds(text) {
    ...
    const m = { second: 1, minute: 60, hour: 3600, day: 86400, week: 604800, month: 2592e3, year: 31536e3 };
    for (const [k, mult] of Object.entries(m)) {
      if (unit.startsWith(k)) return val * mult;
    }
    return val;
  }
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "FETCH_GLOBAL_RESULTS") {
      chrome.storage.local.get("isEnabled", (store) => {
        ...
        processGlobalSearch(message.query, page).then((videos) => sendResponse({ success: true, videos })).catch((err) => {
